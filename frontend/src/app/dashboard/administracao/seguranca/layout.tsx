...
import { Globe, Fingerprint, ScanFace } from 'lucide-react';
...
export default function SegurancaLayout({ children }: { children: React.ReactNode }) {
  ...
  React.useEffect(() => {
    if (pathname === '/dashboard/administracao/seguranca') {
      router.replace('/dashboard/administracao/seguranca/sessoes');
    }
  }, [pathname, router]);

  const TABS = [
    { label: 'Sessões Ativas', href: '/dashboard/administracao/seguranca/sessoes', icon: Globe },
    { label: 'Políticas de MFA', href: '/dashboard/administracao/seguranca/mfa', icon: Fingerprint },
    { label: 'Single Sign-On', href: '/dashboard/administracao/seguranca/sso', icon: ScanFace },
  ];
